...
C Wire chamber resolutions (sigma)
	real*8 hdc_sigma(1:12)/	0.035,0.035,0.035,0.035,0.035,0.035,
...
C Local declarations.
	...
	integer*4 i,j,iplane,jchamber,npl_off
...
C These have to be real*8 for the modified CERNLIB lfit routine.
...
C fit track to give new focal plane values, use LFIT from GENLIB
	do jchamber=1,hdc_nr_cham
     ...
     >		(iplane-0.5-0.5*hdc_nr_plan)*hdc_del_plane
	  enddo
	enddo
	call lfit(zdc,xdc,12,key,dxfp4,xfp4,badf)
	call lfit(zdc,ydc,12,key,dyfp4,yfp4,badf)
